    ...
    <div class="content_box">
      <group :gutter="'20px'">
        ...
        <x-input
          ...
          :required="true"
        ></x-input>
        <x-textarea title="*学习内容反馈" v-model="feedbackContent" placeholder="请输入" :autosize="true"></x-textarea>
        <x-input
          title="默认抄送"
        ...
        >
          <span>抄送至：</span>
          <checker-item :value="
          1" @on-item-click="onItemClick">科室主任</checker-item>
          <checker-item :value="
          2" @on-item-click="onItemClick">科室组长</checker-item>
        </checker>
      </group>
      <div class="button_box">
        <x-button class="edit_btn" @click.native="addSubmit">提交</x-button>
        <x-button class="close_btn" @click.native="addSubmit">保存</x-button>
        <x-button class="close_btn" @click.native="close">关闭</x-button>
      </div>
    </div>
  </div>
</template>
